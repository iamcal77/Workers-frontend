...
const PaymentApproval = () => {
  ...
  useEffect(() => {
    axios.get('https://localhost:7050/api/workers/pending-workers', {
      headers: {
  ...
  const approveFarmer = (id) => {
    axios.patch(`https://localhost:7050/api/workers/${id}/approve-payment`, 
      { status: 'Approved' }, 
      ...
      .catch(() => toast.error('Only admin perfoms the task.'));
  };
  

  const rejectFarmer = (id) => {
    axios.patch(`https://localhost:7050/api/workers/${id}/reject-payment`, 
      { status: 'Rejected' }, 
      { headers: { 'Authorization': `Bearer ${token}` } })
      .then(() => {
